...
function UpdateRequestsPanel({ taskId, onClose }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.get(`/worker/tasks/${taskId}/update-requests`)
  ...
  async function submitResponse(requestId) {
    ...
    try {
      await api.patch(`/worker/tasks/${taskId}/update-requests/${requestId}/respond`, { response: responseText.trim() });
...
function ProgressModal({ task, onClose, onUpdated }) {
  const [status, setStatus] = useState(task.status === 'ASSIGNED' ? 'IN_PROGRESS' : 'COMPLETED');
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState('');
  ...
  async function submit(e) {
    ...
    setSubmitting(true); setError('');
    try {
      await api.patch(`/worker/tasks/${task.task_id}/progress`, { status });
      onUpdated();
      onClose();
    } catch (err) {
...
export default function MyTasks() {
  ...
  async function acknowledge(task) {
    try {
      await api.patch(`/worker/tasks/${task.task_id}/acknowledge`);
      load();
    ...
    <DashboardLayout navItems={WORKER_NAV} roleLabel="Permanent Worker">
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map((t) => (
              <div key={t.task_id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
      ...
      {progressTask && (
        <ProgressModal
          ...
          onUpdated={() => { load(); showToast('Task status updated'); }}
        />
      )}
      {requestsTaskId && (
