...
export default function WorkerDashboard() {
  const { user } = useAuth();
  // A project-based organisation runs no roster for its permanent staff, so there
  // is nothing to clock against and no shift to show. Their day is measured by the
  // tasks they complete instead. Only an explicit PROJECT switches this off — an
  // unknown org type (a session predating the field) keeps the old behaviour, and
  ...
  // The two clock-based tiles have no meaning without a roster, so a project-based
  // worker sees what they have delivered in their place.
  const stats = [
    ...
    ...(isProjectOrg
      ? [{ label: 'COMPLETED TASKS', value: String(completedCount), color: 'text-green-600' }]
      : [
  ...
  const quickActions = [
    ...
    ...(isProjectOrg ? [] : [{ label: 'View Schedule', to: '/worker/calendar' }]),
  ...
  return (
    <DashboardLayout navItems={WORKER_NAV} roleLabel="Permanent Employee" topbarRight={isProjectOrg ? undefined : clockInButton}>
      <div className="space-y-6">
        <div>
          <h2 className="text-xl font-bold text-gray-800">My Day</h2>
          <p className="text-gray-500 text-sm mt-0.5">
            {isProjectOrg ? 'View your tasks and track the work you have delivered.' : 'View your tasks, schedule, and track attendance.'}
          </p>
        </div>
        ...
        {/* Stats */}
        <div className={`grid grid-cols-2 gap-4 ${isProjectOrg ? 'lg:grid-cols-4' : 'lg:grid-cols-5'}`}>
          {stats.map((s) => (
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            {/* Clock card and roster only exist where there is a roster. */}
            {isProjectOrg ? (
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-gray-800 text-sm">Next Deadlines</h3>
                  <Link to="/worker/tasks" className="text-primary-600 text-xs hover:underline">View</Link>
                </div>
                <div className="space-y-2">
                  {tasks
                    .filter((t) => ['ASSIGNED', 'PENDING', 'IN_PROGRESS'].includes(t.status))
                    .sort((a, b) => new Date(a.end_datetime) - new Date(b.end_datetime))
                    .slice(0, 4)
                    .map((t) => (
                      <div key={t.task_id} className="flex items-center justify-between text-sm gap-3">
                        <p className="text-gray-700 font-medium truncate">{t.title}</p>
                        <span className="text-gray-500 text-xs whitespace-nowrap">{fmtDate(t.end_datetime)}</span>
                      </div>
                    ))}
                  {tasks.filter((t) => ['ASSIGNED', 'PENDING', 'IN_PROGRESS'].includes(t.status)).length === 0
                    && <p className="text-gray-400 text-sm">Nothing due.</p>}
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Link to="/worker/calendar" className="text-primary-600 text-xs hover:underline">View</Link>
                  </div>
                  <div className="space-y-2">
