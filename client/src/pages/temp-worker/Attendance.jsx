...
export default function Attendance() {
  ...
  useEffect(() => {
    api.get('/temp-worker/attendance')
  ...
  const handleClockOut = async () => {
    ...
    setError('');
    try {
      const res = await api.put('/temp-worker/attendance/clock-out');
    ...
    <DashboardLayout navItems={TEMP_NAV} roleLabel="Temporary Employee">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Shifts Worked', value: completedRecords.length.toString(), color: 'text-green-600' },
            { label: 'Hours This Month', value: totalHoursDisplay, color: 'text-primary-600' },
            { label: 'Avg Hours / Shift', value: completedRecords.length > 0 ? formatHours(totalMinutes / 60 / completedRecords.length) : '—', color: 'text-gray-700' },
          ].map((s) => (
