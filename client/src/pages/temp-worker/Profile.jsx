...
export default function Profile() {
  ...
  const loadProfile = () => api.get('/temp-worker/profile')
    .then((r) => setProfile(r.data.data ?? null))
    .catch(() => {});

  useEffect(() => {
    api.get('/temp-worker/skills').then((r) => setSkills(r.data.data ?? [])).catch(() => {});
    loadProfile();
  }, []);
  ...
  async function submit(e) {
    ...
    try {
      await api.post('/temp-worker/profile/change-request', {
        field: form.field,
  ...
  return (
    <DashboardLayout navItems={TEMP_NAV} roleLabel="Temporary Employee">
      ...
      {skillModalOpen && (
        <SkillRequestModal
          endpoint="/temp-worker/profile/change-request"
